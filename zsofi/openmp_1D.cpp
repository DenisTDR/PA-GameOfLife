...
void analyze_row(int start, int end){
	for (int i = start; i < end; i++)
	{
		for (int c = 1; c < N - 1; c++){
			new_iteration[i][c] = analyze_cell(i,c);
		}
	}
}
...
void apply_algorithm(int number_threads)
{
	...
	int iam, nt, irows, istart,r;
	
	#pragma omp parallel default(shared) private(iam,nt,irows,istart,r) num_threads(number_threads)
	{
		iam = omp_get_thread_num();
		nt = omp_get_num_threads();
		irows = (N - 2) / nt; /* size of partition */
		istart = iam * irows + 1; /* starting array index */
		if (iam == nt-1) /* last thread may do more */
			irows = N - 1 - istart;
		// for (r = istart; r++; r<istart + irows)
		// {
		// 	cout << "called " << iam << " with row " << r << endl;
		// 	analyze_row(r);
		// }
		analyze_row(istart, istart+irows);
	}
}
...
{
	omp_set_dynamic(0);
	string output_file = "output_openmp_1D.txt";
