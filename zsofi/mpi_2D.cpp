...
int main(int argc, char* argv[])
{
	...
	if (rank==0){
		...
		localinfo[0] = slice_height; localinfo[1] = slice_width;
		string start = "start";
	...
	{	
			...
			for (int z=0; z<r; z++)
			{
				for (int k=0; k<c; k++) 
				{
					for (int l=0; l<sl_height; l++) 
					{
						...
						{	
							if ((z == 0 && l == 0) || (z == r-1 && l == (sl_height - 1)) || (k == 0 && m == 0) || (k == c-1 && m == (sl_width -1))){
								slice[l][m] = 0;
							}
							else{
								slice[l][m]=theBoard[l - 1 + (z*(sl_height-2))][m - 1 + k*(sl_width-2)];	//cut a slice from the the board
							}
						}
					}
			...
			for (int l=0; l<sl_height; l++) 
					{
						...
						{	
							if ((z == 0 && l == 0) || (z == r-1 && l == (sl_height - 1)) || (k == 0 && m == 0) || (k == c-1 && m == (sl_width -1))){
								...
								myslice[l][m]=theBoard[l - 1 + (z*(sl_height-2))][m - 1 + k*(sl_width-2)];	//cut a slice from the the board
							}
						}
					}
		}
		...
		if (rank==0) 
		{
			...
			for (int i=1; i<size; i++)
			{
				...
				MPI_Recv(&aBoard, slice_height*slice_width, MPI_INT, i, 1, MPI_COMM_WORLD, &Stat); //receive all others'
				// cout << "A board" << endl;
				...
				for (int x=1; x<slice_height-1; x++)
				{
					for (int y=1; y<slice_width-1; y++)
					{
						theBoard[row_num * (slice_height - 2) + x-1][col_num * (slice_width-2) + y-1] = aBoard[x][y];		
